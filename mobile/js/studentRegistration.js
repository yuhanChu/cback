...
var app = new Vue({
    ...
    methods: {
        ...
        date: function(res) {
            this.json.start_school = res;
        },
        schoolCh: function(res) {
            this.schoolFlag = res.value;
            if (res.value != 7) {
                this.json.school = res.label
            } else {
                this.json.school = ''
            }
        },
        ...
        submit: function() {
            ...
            if (!this.json.school) {
                promit.msg('app', text.regisration60)
                return
            }
            if (!this.json.start_school) {
                promit.msg('app', text.regisration61)
                return
            }
            if (!this.json.major) {
                promit.msg('app', text.regisration62)
                return
            }
            if (!this.json.class) {
                promit.msg('app', text.regisration63)
                return
            }
            if (!this.json.level_type) {
                promit.msg('app', text.regisration64)
                return
            }
            if (!this.json.system) {
                promit.msg('app', text.regisration65)
                return
            }
            if (!this.json.level) {
                promit.msg('app', text.regisration66)
                return
            }
